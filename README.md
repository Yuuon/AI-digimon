...
| `/evolist` | 进化列表, evo, el | 查看当前可进化的分支选项 |
| `/evoselect` | 进化选择, evos, es | 选择特定的进化分支（后跟序号） |
...
| `/reloadtavern` | 重载酒馆配置 | 【管理员】重新加载酒馆配置文件 |
| `/specialfocus` | 特别关注, sf | 【管理员】管理特别关注用户列表 |
...
酒馆系统的配置文件 `Data/tavern_config.json` 支持热重载，修改后无需重启程序：

```bash
/reloadtavern      # 重新加载酒馆配置
/重载酒馆配置      # 同上
```

**支持热重载的配置项**：
- 监测参数（消息数、关键词阈值、冷却时间等）
- 自主发言设置
- 特别关注列表
- AI生成参数
...
```json
{
  "SpecialFocus": {
    "Enabled": true,
    "UserIds": ["123456789", "987654321@g757123426"],
    "CooldownMinutes": 3,
    "RequireMention": false,
    "MessagePrefix": "🎭 **{CharacterName}**（注意到{UserName}的发言）\n\n",
    "ScenarioTemplate": "{UserName}对你说：{Message}\n\n请根据这段话进行回复。"
  }
}
```

**注意事项**：
- 支持两种用户格式：纯QQ号（所有群生效）或 `QQ号@群ID`（仅指定群生效）
- 单独设置冷却时间，与自主发言冷却独立
- 如果开启 `RequireMention`，则必须@Bot才会回复关注用户

### 完整配置示例

`Data/tavern_config.json`：
...
```json
{
  "Monitor": {
    ...
    "MinKeywordLength": 2,
    "MaxKeywordLength": 10,
    "StopWords": ["的", "了", "是", "我", "你", "他", "在", "有", "和", "就", "都"]
  },
  "AutoSpeak": {
    "Enabled": true,
    "MessagePrefix": "🎭 **{CharacterName}**（听到你们讨论得热烈，忍不住插话）\n\n",
    "ScenarioTemplate": "群聊正在讨论：{Keywords}，请根据这个话题插话参与讨论",
    "MinIntervalSeconds": 10
  },
  "Generation": {
    "SummaryPromptTemplate": "请总结以下群聊对话的主要内容：\n\n{Conversation}\n\n请用2-3句话简洁概括讨论的主题和要点。",
  ...
  "CharacterDirectory": "Data/Characters",
  "EnableDebugLog": true
}
```
